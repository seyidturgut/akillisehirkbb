...
import { MobileARData } from './MobileARData';
import { DesktopDataView } from './DesktopDataView';
import { Portal } from '../../../components/Portal';

interface CityDataOverlayProps {
    onClose: () => void;
}

export const CityDataOverlay: React.FC<CityDataOverlayProps> = ({ onClose }) => {
                ...
                <div className="relative w-full max-w-6xl h-[85vh] md:h-[80vh] bg-gray-900 rounded-[2.5rem] border border-white/10 shadow-[0_0_100px_rgba(0,0,0,0.5)] flex flex-col overflow-hidden animate-in zoom-in-95 duration-500">
                    <div className="flex items-center justify-between p-6 border-b border-white/5">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h2 className="text-xl font-bold text-white leading-tight">Canlı Şehir Verileri</h2>
                                <p className="text-xs text-emerald-400 font-medium tracking-wider uppercase">Gerçek Zamanlı Metrikler</p>
                            </div>
                        </div>
